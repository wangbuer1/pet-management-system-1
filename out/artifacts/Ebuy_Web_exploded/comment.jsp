			...
			<c:forEach items="${commentList}" var="comment">
				<li>
					<dl>
						...
						<dd class="author">
							网友：${comment.nickName} <span class="timer"><fmt:formatDate value="${comment.createTime }" type="date" pattern="yyyy-MM-dd"/></span>
						</dd>
						<c:if test="${not empty comment.replyContent }">
							<dd>客服回复：${comment.replyContent}&nbsp;&nbsp;<span class="timer"><fmt:formatDate value="${comment.replyTime }" type="date" pattern="yyyy-MM-dd"/></span></dd>
						</c:if>
					</dl></li>
					<br/>
			</c:forEach>
